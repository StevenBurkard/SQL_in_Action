...
-- Harlem also has 16 for it's Count so it should be Williamsburg and Harlem.
...
-- <<<<<<<<<<<<<<<<<<<<< ** BONUS ** >>>>>>>>>>>>>>>>>>>>>>>
-- Find if the percent above or below each listing is compared to the average price for all listings.
-- HINT: No hints! It's a bonus for a reason :)
SELECT id, price, host_name,
ROUND((((price - (SELECT AVG(price) FROM final_airbnb)) / (SELECT AVG(price) FROM final_airbnb)) * 100), 2) AS percent
FROM final_airbnb
